...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['add_product'])){
    ...
    $cat_image=trim($_POST['category_image_url'] ?? '');
    $product_image=trim($_POST['image_url'] ?? '');
    $desc=trim($_POST['description']);
    $stock=(int)$_POST['stock'];
    if(!$cat && $new_cat){
        $check=$pdo->prepare("SELECT category_id FROM categories WHERE category_name=?");
        $check->execute([$new_cat]);
        $existing=$check->fetchColumn();
        if($existing){
            $cat=(int)$existing;
            if($cat_image){
                $pdo->prepare("UPDATE categories SET image_url=? WHERE category_id=?")->execute([$cat_image,$cat]);
            }
        } else {
            $pdo->prepare("INSERT INTO categories (category_name,image_url) VALUES (?,?)")->execute([$new_cat,$cat_image]);
            $cat=(int)$pdo->lastInsertId();
        }
    }
    if($name&&$price>0&&$cat){
        $pdo->prepare("INSERT INTO products (seller_id,category_id,name,description,price,stock,image_url) VALUES (?,?,?,?,?,?,?)")
            ->execute([$seller_id,$cat,$name,$desc,$price,$stock,$product_image]);
...
$orders=$pdo->prepare("
  SELECT DISTINCT o.order_id,o.total_amount,o.status,o.payment_ref,o.delivery_notes,o.created_at,u.full_name as buyer_name
  FROM orders o
...
$order_items=$pdo->prepare("
  SELECT o.payment_ref, p.name, oi.quantity, oi.price, oi.special_instructions, u.full_name AS buyer_name, o.created_at
  FROM order_items oi
  ...
  LIMIT 20
");
$order_items->execute([$seller_id]);
$order_items=$order_items->fetchAll(PDO::FETCH_ASSOC);

$messages=$pdo->prepare("
  SELECT m.*, u.full_name AS buyer_name, p.name AS product_name
  FROM messages m
  JOIN users u ON m.buyer_id=u.user_id
  JOIN products p ON m.product_id=p.product_id
  WHERE m.seller_id=?
  ORDER BY m.created_at DESC
  LIMIT 10
");
$messages->execute([$seller_id]);
$messages=$messages->fetchAll(PDO::FETCH_ASSOC);

$payouts=$pdo->prepare("SELECT SUM(gross_amount) gross, SUM(platform_fee) fees, SUM(seller_amount) seller_total FROM seller_payouts WHERE seller_id=?");
$payouts->execute([$seller_id]);
$payouts=$payouts->fetch(PDO::FETCH_ASSOC);
...
<div class="dash-wrap">
  ...
  <div class="stats-row">
    ...
    <div class="stat-card"><div class="num">R<?=number_format($payouts['seller_total'] ?? 0,2)?></div><div class="label">Seller Payouts</div></div>
  </div>
  ...
  <div class="two-col">
    <div class="col-main">
      ...
      <div class="section-box">
        <h3>Buyer Order Instructions</h3>
        <?php if(empty($order_items)): ?>
        <p style="color:var(--muted);font-size:14px;">No item notes yet.</p>
        <?php else: ?>
        <table class="prod-table">
          <thead><tr><th>Ref</th><th>Buyer</th><th>Item</th><th>Qty</th><th>Instruction</th></tr></thead>
          <tbody>
          <?php foreach($order_items as $i): ?>
          <tr>
            <td><?=e($i['payment_ref'])?></td>
            <td><?=e($i['buyer_name'])?></td>
            <td><?=e($i['name'])?></td>
            <td><?=$i['quantity']?></td>
            <td><?=e($i['special_instructions'] ?: 'No special instructions')?></td>
          </tr>
          <?php endforeach; ?>
          </tbody>
        </table>
      ...
      <div class="section-box">
        ...
        <div style="border-bottom:1px solid #eee;padding:10px 0;font-size:13px;">
          <strong><?=e($m['buyer_name'])?></strong> about <strong><?=e($m['product_name'])?></strong><br>
          <span style="color:var(--muted);"><?=e($m['message'])?></span>
        </div>
        <?php endforeach; ?>
        <?php endif; ?>
      </div>
    ...
    <div class="col-side">
      ...
      <div class="section-box">
        ...
        <form method="POST">
          ...
          <div class="form-group"><label>New Category Name</label><input type="text" name="new_category" placeholder="Only if your category is not listed"></div>
          <div class="form-group"><label>Category Image URL</label><input type="url" name="category_image_url" placeholder="Image shown for this category on browse"></div>
          <div class="form-group"><label>Product Image URL</label><input type="url" name="image_url" placeholder="Optional image for this exact item"></div>
