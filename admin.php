...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(isset($_POST['update_dispute_status'])){
        $pdo->prepare("UPDATE disputes SET status=? WHERE dispute_id=?")->execute([$_POST['new_status'],(int)$_POST['dispute_id']]);
    }
...
$disputes=$pdo->query("
  SELECT d.*, b.full_name AS buyer_name, s.full_name AS seller_name, p.name AS product_name, o.payment_ref
  FROM disputes d
  JOIN users b ON d.buyer_id=b.user_id
  JOIN users s ON d.seller_id=s.user_id
...
<div class="admin-wrap">
  ...
  <div class="tabs">
    ...
    <button class="tab-btn <?=$tab==='disputes'?'active':''?>" onclick="switchTab('disputes')">Disputes <?php if(count($disputes)>0): ?><span class="badge-count"><?=count($disputes)?></span><?php endif; ?></button>
  </div>

  <!-- USERS TAB -->
  <div id="tab-users" class="tab-content <?=$tab==='users'?'active':''?> section-box">
    <table>
      <thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Seller Verification</th><th>Joined</th><th>Actions</th></tr></thead>
      <tbody>
      ...
      <tr>
        ...
        <td style="max-width:240px;">
          ...
          <strong><?=htmlspecialchars($u['business_name'] ?? '')?></strong><br>
          <?=htmlspecialchars($u['phone'] ?? '')?> <?=htmlspecialchars($u['location'] ?? '')?><br>
          <small><?=htmlspecialchars($u['id_number'] ?? '')?></small><br>
          <small><?=htmlspecialchars($u['verification_details'] ?? '')?></small>
          <?php else: ?>
          <span style="color:var(--muted);font-size:12px;">Not a seller</span>
          <?php endif; ?>
        </td>
  ...
  <div id="tab-disputes" class="tab-content <?=$tab==='disputes'?'active':''?> section-box">
    <table>
      <thead><tr><th>Order</th><th>Buyer</th><th>Seller</th><th>Product</th><th>Reason</th><th>Description</th><th>Status</th><th>Update</th></tr></thead>
      <tbody>
      ...
      <tr>
        ...
        <td style="max-width:260px;"><?=htmlspecialchars($d['description'])?></td>
        ...
        <td>
          <form method="POST" style="display:flex;gap:5px;align-items:center;">
            ...
            <select name="new_status" class="status-sel">
              ...
              <option value="investigating" <?=$d['status']==='investigating'?'selected':''?>>Investigating</option>
            ...
            <button type="submit" name="update_dispute_status" class="approve-btn">Save</button>
          </form>
        </td>
      </tr>
      <?php endforeach; ?>
      <?php if(empty($disputes)): ?><tr><td colspan="8" style="color:var(--muted);">No disputes have been reported.</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
